...
export default function EditPlayerProfile() {
  ...
  return <SafeAreaView className="flex-1 bg-[#F8F9FA]"><ScrollView className="px-6" contentContainerStyle={{ paddingBottom: 32 }}><View className="flex-row items-center py-4"><TouchableOpacity accessibilityRole="button" accessibilityLabel="Go back" onPress={() => router.back()} className="w-11 h-11 rounded-full bg-white border border-[#E5E5E5] items-center justify-center mr-3"><Text className="text-[#1A1A2E] text-xl">‹</Text></TouchableOpacity><Text className="text-xl font-bold text-[#1A1A2E] flex-1">Edit Profile</Text></View><Text className="text-[#1A1A2E] font-medium mb-2">Full name</Text><TextInput accessibilityLabel="Full name" value={name} onChangeText={setName} className={input} /><Text className="text-[#1A1A2E] font-medium mt-4 mb-2">City</Text><TextInput accessibilityLabel="City" value={city} onChangeText={setCity} className={input} /><Text className="text-[#1A1A2E] font-medium mt-4 mb-2">Bio</Text><TextInput accessibilityLabel="Bio" value={bio} onChangeText={setBio} className={input} multiline /><Text className="text-[#737373] text-xs mt-4">Avatar upload will be available after cloud storage is configured.</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Save profile changes" disabled={saving} onPress={save} className={`rounded-xl py-4 items-center mt-6 ${saving ? 'bg-[#9CA3AF]' : 'bg-[#4CAF50]'}`}>{saving ? <ActivityIndicator color="white" /> : <Text className="text-white font-bold">Save Changes</Text>}</TouchableOpacity></ScrollView><Toast message={toast} tone="error" onHide={() => setToast(null)} /></SafeAreaView>;
}
