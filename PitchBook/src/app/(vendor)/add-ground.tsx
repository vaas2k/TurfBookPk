...
export default function AddGround() {
  ...
  const renderTags = (key: 'amenities' | 'rules') => <View className="flex-row flex-wrap mt-2">{form[key].map((item) => <View key={item} className="flex-row items-center bg-[#E8F5E9] rounded-full px-3 py-2 mr-2 mb-2"><Text className="text-[#2E7D32] mr-2">{item}</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel={`Remove ${item}`} className="h-8 w-8 items-center justify-center" onPress={() => removeTag(key, item)}><Ionicons name="close-circle" size={16} color="#2E7D32" /></TouchableOpacity></View>)}</View>;
  const tagInput = (key: 'amenities' | 'rules', value: string, setValue: (value: string) => void, placeholder: string) => <View className="flex-row items-center"><TextInput accessibilityLabel={placeholder} className={`${fieldClass} flex-1`} value={value} onChangeText={setValue} placeholder={placeholder} placeholderTextColor="#A3A3A3" onSubmitEditing={() => addTag(key, value, setValue)} returnKeyType="done" /><TouchableOpacity accessibilityRole="button" accessibilityLabel={`Add ${key === 'amenities' ? 'amenity' : 'rule'}`} className="bg-[#1A1A2E] rounded-xl h-12 w-12 ml-2 items-center justify-center" onPress={() => addTag(key, value, setValue)}><Ionicons name="add" size={22} color="white" /></TouchableOpacity></View>;

  return <SafeAreaView className="flex-1 bg-[#F8F9FA]"><ScrollView className="flex-1 px-6" contentContainerStyle={{ paddingBottom: 40 }}>
    <View className="flex-row items-center py-5"><TouchableOpacity accessibilityRole="button" accessibilityLabel="Go back" className="h-11 w-11 rounded-full bg-white border border-[#E5E5E5] items-center justify-center" onPress={() => goBackOrReplace('/(vendor)/grounds')}><Ionicons name="arrow-back" size={20} color="#1A1A2E" /></TouchableOpacity><Text className="text-xl font-bold text-[#1A1A2E] flex-1 text-center">{existing ? 'Edit Ground' : 'Add Ground'}</Text><View className="w-11" /></View>
